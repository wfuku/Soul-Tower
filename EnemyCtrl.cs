...
public class EnemyCtrl : MonoBehaviour
{
    ...
    void Walking()
    {
        ...
        if (waitTime > 0.0f)
        {
            ...
            if (waitTime <= 0.0f)
            {
                ...
                walkTime = Random.Range(waitBaseTime, waitBaseTime * 2.0f);
            }
        }
        else
        {
    ...
    void Chasing()
    {
        ...
        // 2m以内に近づいたら攻撃
        if (tag == "Boss" || tag == "Large")
        {

            if (Vector3.Distance(attackTarget.position, transform.position) <= 4.0f)
            {
                ChangeState(State.Attacking);
                status.attackNumber = 0;
            }
            else if (Vector3.Distance(attackTarget.position, transform.position) <= 9.0f && status.longAttackHave == true)
            {
                ChangeState(State.Attacking);
                status.attackNumber = 1;
            }
        ...
        else
        {
            ...
            else if (Vector3.Distance(attackTarget.position, transform.position) <= 7.0f && status.longAttackHave == true)
            {
                ChangeState(State.Attacking);
                status.attackNumber = 1;
            }
    ...
    void dropItem()
    {
        ...
        Instantiate(dropItem,dropPoint.transform.position, Quaternion.identity);
    }
    ...
    public void SetLongAttackTarget(Transform target)
    {
        attackTarget = target;
    }
}